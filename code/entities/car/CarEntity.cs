...
public partial class CarEntity : Prop, IUse
{
	...
	protected override void OnDestroy()
	{
		...
		if ( IsServer && Driver is MyPlayer player )
		{
			RemoveDriver( player );
		}
	}
	...
	protected void Tick()
	{
		if ( Driver is MyPlayer player && player.LifeState != LifeState.Alive )
		{
			RemoveDriver( player );
		}
	}
	...
	void SimulateDriver( Client client )
	{
		...
		if ( IsServer )
		{
			...
			{
				RemoveDriver( Driver as MyPlayer );
	...
	private void RemoveDriver( MyPlayer player )
	{
		if ( !IsServer )
			return;
	...
	{
		if ( user is MyPlayer player && timeSinceDriverLeft > 1.0f )
		{
			player.Parent = this;
	...
	protected override void OnPhysicsCollision( CollisionEventData eventData )
	{
		...
		if ( speed > minImpactSpeed )
		{
			if ( other.Entity.IsValid() && other.Entity != this )
			{
				...
				if ( other.Entity.LifeState == LifeState.Dead && other.Entity is not MyPlayer )
				{
